...
class SystemModuleFixup:
    ...
    def relocate_pointer(self, module_base: int, address: int) -> int:
        ...
        # print(f"relocate_pointer address: 0x{address:x}")
    ...
    def relocate_symbols(self, binary: lief.MachO.Binary, module_base: int):
        ...
        for symbol in binary.symbols:
            if not symbol.value or symbol.value < 0x100000000:
                continue
    ...
    def fixup_sections(self, binary: lief.MachO.Binary, module_base: int):
        ...
        for section_name in section_names:
            ...
            # print(f"begin: 0x{begin:x} end: 0x{end:x} values: {values}")

            if section_name == "__objc_classlist":
                # print(f"fixup section_name {section_name} begin 0x{begin:x} end 0x{end:x}")
                for value in values:
    ...
    def fixup_method_list(self, module_base: int, address: int):
        ...
        if flag < 0x80000000:
            ...
            for i in range(count):
                ...
                sel_offset = self.emu.read_s32(sel_offset_ptr)

                # print(f"module_base {module_base} sel_offset_ptr 0x{sel_offset_ptr:x} sel_offset 0x{sel_offset:x}")
                # Fixup method SEL
    ...
    def fixup_variable_list(self, module_base: int, address: int):
        ...
        if not variable_list_ptr:
            return
        # print(f"variable_list_ptr {variable_list_ptr}")
    ...
    def fixup_class_struct(self, module_base: int, address: int):
        ...
        # print(f"address + 32: 0x{address + 32:x} data_ptr 1 0x{data_ptr:x}")
        self.add_refs_relocation(data_ptr)

        data_ptr += module_base
        # print(f"data_ptr + 24: 0x{data_ptr + 24:x} data_ptr 2 0x{data_ptr:x}")
        ...
        # print(f"protocols_ptr: 0x{protocols_ptr:x}")
        self.fixup_protocol_list(module_base, protocols_ptr)

        instance_variables_ptr = data_ptr + 48
        # print(f"instance_variables_ptr: 0x{instance_variables_ptr:x}")
    ...
    def install(self, module: Module):
        # print(f"fixup.install Module.name: {module.name}")
